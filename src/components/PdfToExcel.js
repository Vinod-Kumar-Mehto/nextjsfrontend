...
import wordthumnail from "@/assets/images/excel.png";
...
const PdfToExcel = () => {
  ...
  const t = useTranslations("pdfToExcelConverter");
  ...
  const requestMade = async () => {
    ...
    try {
      ...
      for (let i = 0; i < selectedPdf.length; i++) {
        formData.append("pdf", selectedPdf[i].fileData);
      }

      const response = await axios.post(
        `${process.env.NEXT_PUBLIC_PORT}/pdftoexcel`,
        formData,
        {
          headers: {
